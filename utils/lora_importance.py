...
class RankAllocator(object):
    ...
    def set_total_step(self, total_step:int): 
        ...
        self.total_step = total_step
        #self.initial_warmup = int(self.total_step / 5) + 1
    ...
    def update_ipt(self, model, global_step): 
        for n,p in model.named_parameters():
            if "lora_" in n: 
                if p.grad is None:
                    print(f"Gradient for parameter {n} is None")
                elif torch.isnan(p.grad).any():
                    continue
                    print(f"Gradient for parameter {n} contains NaN values")
                if torch.isnan(p).any() or torch.isinf(p).any():
                    continue
                    print(f"Parameter {n} contains NaN or Inf values")
                if not p.requires_grad:
                    continue
                    print(f"Parameter {n} does not require gradient")
                ...
                with torch.no_grad():
                    ...
                    if torch.isnan(self.ipt[n]).any():
                        print(f"ipt {n} contains NaN or Inf values")
                        print(p.dtype, p.grad.dtype, p.device, p.grad.device)
                        print(p.max(), p.min(), p.grad.max(), p.grad.min())
                        self.ipt[n] = torch.zeros_like(p)
                ...
                print(f"step is {global_step} total step is {self.total_step}")
                self.calculate_score(metric="ipt")
                # print(f"name is {n}")
    ...
    def calculate_score(self, p=None, metric="ipt"):
        ...
        for n in self.exp_avg_ipt:
            ...
            layer = re.findall(r'layers\.(\d+)\.', n)[0]
            if metric == "ipt":
                # Combine the senstivity and uncertainty 
                ipt_score = self.layer_weights[int(layer)]*self.exp_avg_ipt[n] * self.exp_avg_unc[n]
            elif metric == "mag":
        ...
        print(f"mean scores are {ipt_score_list}")
